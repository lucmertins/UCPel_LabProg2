...
public class LoginController5 {
    ...
    public LoginController5(HttpServletRequest req, HttpServletResponse resp, Connection conn) {
        this.req = req;
        this.resp = resp;
        this.conn = conn;
    }
    ...
    public void processo() throws ServletException, IOException, SQLException {
        ...
        if (usuario == null || !usuario.validaSenha(senha)) {
            ServletPrincipal5.dispatcherErro(req, resp, String.format("Usuário ou Senha Inválidos.[%s]", login));
            return;
        } else {
